      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="paper overflow-hidden">
          <div className="relative aspect-[16/8] overflow-hidden bg-[linear-gradient(135deg,#f3ead9,#dbc298)]">
            {post.featured_image_url ? (
              ...
              <div className="absolute inset-0 flex items-end p-10">
                <p className="display-font text-5xl font-semibold text-slate-900/80">{post.title}</p>
              </div>
            )}
          </div>
          ...
          <div className="p-6 sm:p-10">
            <div className="mb-6 flex flex-wrap items-center gap-3 text-sm font-medium uppercase tracking-[0.18em] text-slate-500">
              ...
              {post.published_at ? <span>{format(new Date(post.published_at), 'MMMM d, yyyy')}</span> : null}
              <span>{post.reading_time_minutes} min read</span>
            </div>

            <h1 className="display-font text-4xl font-semibold tracking-tight text-slate-900 sm:text-5xl lg:text-6xl">{post.title}</h1>
            <p className="mt-5 max-w-3xl text-xl leading-9 text-slate-600">{post.excerpt}</p>
            ...
            <div className="mt-12 border-t border-slate-200 pt-8">
              <Link href="/guest-post" className="text-sm font-semibold text-brand-700 hover:text-brand-900">
                Enjoyed this piece? Pitch an article for the journal →
              </Link>
            </div>
          </div>
        </div>
        ...
        <aside className="space-y-6">
          ...
          <div className="paper p-6">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand-700">Editorial standard</p>
            <p className="mt-3 text-sm leading-7 text-slate-600">
              Articles on this site should be specific, evidence-aware, and written in a voice that sounds like a practitioner or editor.
            </p>
          </div>
        </aside>
      </div>
