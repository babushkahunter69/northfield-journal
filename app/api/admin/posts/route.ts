...
import { estimateReadingTime, excerptFromContent, makeSlug, splitKeywords } from '@/lib/utils';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = {
      title,
      slug,
      excerpt: String(body.excerpt || '').trim() || excerptFromContent(content, 180),
      content,
    ...
    if (body.id) {
      const existingResponse = await supabaseAdmin.from('posts').select('published_at').eq('id', body.id).single();
